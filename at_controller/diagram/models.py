from at_controller.diagram.state import State, LinkTransition, FrameHandlerTransition, EventTransition, Diagram, SetAttributeAction, GetAttribute, FrameUrl, AuthToken, Frame, EquatationCondition, InclusionCondition, AndCondition, OrCondition, NotCondition, NonArgOperationCondition, BinaryOperationCondition
from pydantic import BaseModel, field_validator, RootModel, ValidationError, Field, model_validator
...
class AndConditionModel(BaseModel):
    ...
    conditions: List[Union[
        'EquatationConditionModel', 'InclusionConditionModel', 
        'AndConditionModel', 'OrConditionModel', 'NotConditionModel'
    ]]
...
class NonArgOperationConditionModel(BaseModel):
    type: Literal[
        'len', 'sqrt', 'abs', 'ceil', 'floor', 'round', 'sign', 
        'log', 'exp', 'sin', 'cos', 'tan', 'asin', 'acos', 
        'atan', 'neg', 'transpose', 'det', 'inv', 'norm', 
        'trace', 'is_null'
    ]
    ...
    def parse_conditions(cls, values):
        if values.get('condition'):
            values['condition'] = EventTransitionModel.parse_condition(values['condition'])
        return values

    def to_internal(self) -> 'NonArgOperationCondition':
        return NonArgOperationCondition(type=self.type, condition=self.condition.to_internal() if self.condition else None)

class BinaryOperationConditionModel(BaseModel):
    type: Literal[
        'add', 'sub', 'mul', 'div', 'mod', 'pow', 'logical_and', 
        'logical_or', 'xor', 'max', 'min', 'equal', 'not_equal', 
...
class EventTransitionModel(BaseModel):
    ...
    trigger_condition: Optional[Union[
        EquatationConditionModel, InclusionConditionModel, 
        AndConditionModel, OrConditionModel, NotConditionModel, 
        NonArgOperationConditionModel, BinaryOperationConditionModel
    ]]
...
class Transitions(RootModel[Dict[str, Union[LinkTransitionModel, FrameHandlerTransitionModel, EventTransitionModel]]]):
    @classmethod
